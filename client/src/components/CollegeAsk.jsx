...
const CollegeAsk = () => {
  ...
  try {
    ...
    if (res.status === 429) {
      ...
      toast.error(error || "API usage limit reached for today", {
        position: "top-right",
        autoClose: 5000,
      ...
      setAnswer("🚫 You’ve reached today’s limit. Try again tomorrow.");
      speak("You’ve reached today’s limit. Try again tomorrow.");
       setLoading(false);
      return;
    }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="btn-group">
            <button type="submit" className="ask-btn">✅ Ask</button>
             <button type="button" className="stop-btn" onClick={stopSpeaking}>🛑 Stop</button>
          </div>
        </form>

     <div className="response-box">
        <h3>🧠 Answer:</h3>
        {loading ? <div className="loader">Fetching response...</div> : <p>{answer}</p>}
      </div>
      </div>
    </>
  );
