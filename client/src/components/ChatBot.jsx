...
export default function ChatBot({ inputText, onResponse, onSpeech }) {
  ...
  const queryGemini = async (text) => {
    setLoading(true);
    try {
      const response = await fetch(`${process.env.REACT_APP_BASE_URL}/college-ask`, {
        method: "POST",
      ...
      onResponse(result || "No response from backend.");
    } catch (err) {
      console.error("Error contacting backend:", err);
      onResponse("Error fetching response from server.");
    } finally {
      setLoading(false);
    }
  };
